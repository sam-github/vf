...
of a framework, see iomanager2 (I<ftp://ftp.qnx.com/usr/free/qnx4/os/samples/misc/iomanager2.tgz>).
My framework redoes some of
their work, but also does things theirs doesn't.
I'll probably be reading the new example in detail soon,
...
support that already exists in the socket++ (I<http://toronto.qenesis.com/Sam/socket++.html>)
...
Before I go back to the ftp filesystem I want to multi-thread
the pop mail download with local system calls made on already
downloaded files, and I want to read gzipped tar files. Not
hard, but I don't ever have enough time.
...
feel free to contact me (I<mailto:sam@cogent.ca>), I'd like to
help.
...
You'll need socket++ (I<http://toronto.qenesis.com/Sam/socket++-1.10.1.tgz>) to
compile the pop3 filesystem.

=item *

Source for all the virtual filesystems (I<http://toronto.qenesis.com/Sam/vf-1.0.tgz>)

=item *

Binaries: Remember, these I<must> be run as root (or set-uid root) so you
may wish to download and look over the source yourself.

=over 8

=item *

vf_ram (I<http://toronto.qenesis.com/Sam/vf_ram.gz>)

=item *

vf_tar (I<http://toronto.qenesis.com/Sam/vf_tar.gz>)

=item *

vf_pop (I<http://toronto.qenesis.com/Sam/vf_pop.gz>)
